...
def get_nyt(metric="cases"):
    ...
    df["loc"] = df["subregional1_name"] + "_"+df["name"]
    pivot = df.pivot_table(value=metric, columns=["loc"], index="date")
    ...
    df = df.pivot_table(values=metric, columns=["loc"], input="date")
    ...
    #Only NYT data up to the date for which we have nystate data
...
def get_jhu(metric="cases"):
    ...
    data_cols = [ c for c in merged.columns if re.match("\d+/\d+/\d+", c)]
    transposed = merged[data_cols + ["loc"]].set_index("loc").transpose()
